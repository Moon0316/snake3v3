...
from algo.network_sac import Actor, Critic
...
class SAC:
    ...
    def __init__(self, obs_dim, act_dim, num_agent, args):
        ...
        self.a_lr = args.a_lr
        self.c_lr = args.c_lr
        ######################## extra parameters for SAC #############################
        self.alpha_lr = args.a_lr
        ...
        self.critic_1 = Critic(obs_dim, act_dim, num_agent, args).to(self.device)
        self.critic_2 = Critic(obs_dim, act_dim, num_agent, args).to(self.device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.a_lr)
        self.critic_optimizer_1 = torch.optim.Adam(self.critic_1.parameters(), lr=self.c_lr)
        self.critic_optimizer_2 = torch.optim.Adam(self.critic_2.parameters(), lr=self.c_lr)
        
        # Initialise target network and critic network with ξ' ← ξ and θ' ← θ
        # seems that the actor target network is not used
        # self.actor_target = Actor(obs_dim, act_dim, num_agent, args, self.output_activation).to(self.device)
        self.critic_target_1 = Critic(obs_dim, act_dim, num_agent, args).to(self.device)
        self.critic_target_2 = Critic(obs_dim, act_dim, num_agent, args).to(self.device)
        # hard_update(self.actor, self.actor_target)
        hard_update(self.critic_1, self.critic_target_1)
        hard_update(self.critic_2, self.critic_target_2)
    ...
    def choose_action(self, obs, evaluation=False):
        ...
        if p > self.eps or evaluation:
            ...
            action = self.actor(obs)[0].cpu().detach().numpy()[0]
        else:
    ...
    def update(self):
        ...
        states = torch.Tensor(state_batch).reshape(self.batch_size, self.num_agent, -1).to(self.device)
        actions = torch.Tensor(action_batch).reshape(self.batch_size, self.num_agent, -1).to(self.device)
        rewards = torch.Tensor(reward_batch).reshape(self.batch_size, self.num_agent, 1).to(self.device)
        next_states = torch.Tensor(next_state_batch).reshape(self.batch_size, self.num_agent, -1).to(self.device)
        dones = torch.Tensor(done_batch).reshape(self.batch_size, self.num_agent, 1).to(self.device)
        # update 2 critic Q networks
        ...
        self.c_loss = (critic_1_loss.item()+critic_2_loss.item())/2
    ...
    def load_model(self, run_dir, episode):
        ...
        model_critic_1_path = os.path.join(base_path, "critic_1_" + str(episode) + ".pth")
        model_critic_2_path = os.path.join(base_path, "critic_2_" + str(episode) + ".pth")
        print(f'Actor path: {model_actor_path}')
        print(f'Critic_1 path: {model_critic_1_path}')
        print(f'Critic_2 path: {model_critic_2_path}')

        if os.path.exists(model_critic_1_path) and os.path.exists(model_actor_path) and os.path.exists(model_critic_2_path):
            actor = torch.load(model_actor_path, map_location=device)
            critic_1 = torch.load(model_critic_1_path, map_location=device)
            critic_2 = torch.load(model_critic_2_path, map_location=device)
            self.actor.load_state_dict(actor)
            self.critic_1.load_state_dict(critic_1)
            self.critic_2.load_state_dict(critic_2)
            print("Model loaded!")
        else:
