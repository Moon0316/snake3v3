...
import torch
import random
from submission_files.submission_sac.submission import agent as sac_agent
from submission_files.submission_sac.submission import get_observations as sac_get_observations
from submission_files.submission_qmix.submission import agent as qmix_agent
from submission_files.submission_qmix.submission import get_observations as qmix_get_observations
from submission_files.submission_ddpg.submission import agent as ddpg_agent
from submission_files.submission_ddpg.submission import get_observations as ddpg_get_observations
...
def get_actions(state, algo, indexs):
    ...
    # rl agent
    if algo == 'sac':
        obs = sac_get_observations(state, indexs, obs_dim=26, height=10, width=20)
        logits = sac_agent.choose_action(obs)
        logits = torch.Tensor(logits)
        actions = np.array([Categorical(out).sample().item() for out in logits])
        
    if algo == 'ddpg':
        obs = ddpg_get_observations(state, indexs, obs_dim=26, height=10, width=20)
        logits = ddpg_agent.choose_action(obs)
        logits = torch.Tensor(logits)
        actions = np.array([Categorical(out).sample().item() for out in logits])

    if algo == 'qmix':
        obs = qmix_get_observations(state, indexs, obs_dim=26, height=10, width=20)
        logits = qmix_agent.choose_action(obs)
        logits = torch.Tensor(logits)
        actions = np.array([Categorical(out).sample().item() for out in logits])    
    return actions
...
if __name__ == "__main__":
    ...
    parser.add_argument("--my_ai", default="ddpg", help="sac/ddpg/random/qmix")
    parser.add_argument("--opponent", default="sac", help="sac/ddpg/random/qmix")
